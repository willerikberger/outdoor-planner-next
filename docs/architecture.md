  ...
  ClientLoader (dynamic import, ssr: false)
    PlannerApp
      ...
      PlannerCanvas (forwardRef, imperative handle)
        ...
        useKeyboardShortcuts  -- Delete, Escape handlers
      StatusBar
```
...
4. Only runs if `autoSaveEnabled` is true in the store
...
4. Wrap in `SerializedProject` (version 2, timestamp, background image data)
