...
import { createMaskRect } from '@/components/canvas/utils/fabricHelpers'
...
export function useCleanup(
  ...
  const enterCleanupMode = useCallback(() => {
    ...
    for (const obj of store.objects.values()) {
      for (const fo of canvas.getObjects()) {
        ...
        if (foAny.parentId != null) {
          const parent = store.objects.get(foAny.parentId as number)
          if (parent && parent.type !== 'mask' && parent.type !== 'backgroundImage') {
            fo.set('visible', false)
          }
        }
      }
  ...
  const exitCleanupMode = useCallback(() => {
    ...
    for (const fo of canvas.getObjects()) {
      fo.set('visible', true)
      const foAny = fo as unknown as Record<string, unknown>
      if (foAny.objectType === 'mask' || foAny.objectType === 'backgroundImage') {
  ...
  const startDrawingMask = useCallback(() => {
    ...
    usePlannerStore.getState().setStatusMessage('Click and drag to draw a mask rectangle')
  }, [fabricCanvasRef])
  ...
  const finishMaskRect = useCallback(() => {
    ...
    ;(mask as unknown as Record<string, unknown>).objectId = id
    ;(mask as unknown as Record<string, unknown>).objectType = 'mask'
  ...
  const addCleanupImage = useCallback(
    (file: File) => {
      ...
      reader.onload = async (e) => {
        ...
        const store = usePlannerStore.getState()
        const id = store.nextObjectId()
        ;(img as unknown as Record<string, unknown>).objectId = id
        ;(img as unknown as Record<string, unknown>).objectType = 'backgroundImage'
        ;(img as unknown as Record<string, unknown>).imageData = dataUrl
