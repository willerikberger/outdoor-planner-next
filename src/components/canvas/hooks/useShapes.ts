...
import type { Canvas, Rect, FabricText } from 'fabric'
...
export function useShapes(
  ...
  const updateShapeLabels = useCallback(
    (rect: Rect) => {
      const id = (rect as unknown as Record<string, unknown>).objectId as number
  ...
  const updateShapeDimensions = useCallback(
    (rect: Rect, finalize: boolean) => {
      ...
      const baseWidthPx = (r.baseWidthPx as number) ?? rect.width
      const baseHeightPx = (r.baseHeightPx as number) ?? rect.height
      const currentWidthPx = baseWidthPx * scaleX
      const currentHeightPx = baseHeightPx * scaleY
      const newWidthM = roundToDecimal(currentWidthPx / store.pixelsPerMeter, 1)
      const newHeightM = roundToDecimal(currentHeightPx / store.pixelsPerMeter, 1)

      const id = r.objectId as number
      const refs = fabricRefsRef.current.get(id)
      ...
      if (finalize) {
        store.updateObject(id, { widthM: newWidthM, heightM: newHeightM } as Partial<{ widthM: number; heightM: number }>)
      }
    },
    [fabricRefsRef],
  )
