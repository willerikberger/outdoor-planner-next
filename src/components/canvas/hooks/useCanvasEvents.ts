...
export function useCanvasEvents(
  ...
  const handleObjectModified = useCallback(
    (opt: { target: unknown }) => {
      const obj = opt.target as unknown as Record<string, unknown>
      if (obj?.objectType === 'shape') {
        handlers.updateShapeDimensions(obj as unknown as Rect, true)
        handlers.updateShapeLabels(obj as unknown as Rect)
      }
      if (obj?.objectType === 'line') {
        handlers.updateLineLabel(obj as unknown as Line)
      }
  ...
  const handleObjectScaling = useCallback(
    (opt: { target: unknown }) => {
      const obj = opt.target as unknown as Record<string, unknown>
      if (obj?.objectType === 'shape') {
        handlers.updateShapeDimensions(obj as unknown as Rect, false)
        handlers.updateShapeLabels(obj as unknown as Rect)
      }
    },
    [handlers],
