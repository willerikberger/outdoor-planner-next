...
import { fitImageScale, overlayImageScale } from '@/components/canvas/utils/geometry'
...
export function useImages(
  ...
  const loadBackgroundFromData = useCallback(
    async (dataUrl: string, callback?: () => void) => {
      ...
      const scale = fitImageScale(img.width!, img.height!, canvasWidth, canvasHeight)
      ...
      usePlannerStore.getState().setStatusMessage('Image loaded. Set the scale to start adding shapes.')
      callback?.()
    },
    [fabricCanvasRef],
  )
  ...
  const addOverlayImage = useCallback(
    (file: File) => {
      ...
      reader.onload = async (e) => {
        ...
        ;(img as unknown as Record<string, unknown>).objectType = 'overlayImage'
  ...
  const loadImageObject = useCallback(
    async (data: {
      ...
      const id = store.nextObjectId()
      ;(img as unknown as Record<string, unknown>).objectId = id
      ;(img as unknown as Record<string, unknown>).objectType = data.type
      ;(img as unknown as Record<string, unknown>).imageData = data.imageData
