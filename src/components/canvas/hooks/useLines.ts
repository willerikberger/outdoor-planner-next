...
export function useLines(
      ...
      for (const obj of store.objects.values()) {
        if (obj.type !== 'mask' && obj.type !== 'backgroundImage') {
          for (const fo of canvas.getObjects()) {
            if ((fo as unknown as Record<string, unknown>).objectId === obj.id) {
  ...
  const startLineDrawing = useCallback(() => {
    ...
    usePlannerStore.getState().setStatusMessage('Click and drag to draw. Lines snap to 45\u00b0 angles.')
  ...
  const updateDrawingLine = useCallback(
    (pointer: Point) => {
      ...
      const store = usePlannerStore.getState()
      const meterLen = store.pixelsPerMeter ? roundToDecimal(pxLen / store.pixelsPerMeter, 1) : 0
  ...
  const finishDrawingLine = useCallback(() => {
    ...
    store.setStatusMessage(`Added "${lineName}" (${meterLen}m) - Click to draw another line`)
  }, [fabricCanvasRef, fabricRefsRef])

  const updateLineLabel = useCallback(
    (line: Line) => {
      const id = (line as unknown as Record<string, unknown>).objectId as number
